...
int main()
{
	...
	// 1-arm
	double si_de_th_x = 9.3E-6 / sqrt(2.);
	double si_de_th_y = 3.1E-6 / sqrt(2.);

	double lcut_th_y_L = 28E-6;
	double lcut_th_y_R = 28E-6;
	double hcut_th_y_L = 102E-6;
	double hcut_th_y_R = 102E-6;
	...
	TH2D *h_de_t_vs_t = new TH2D("h_de_t_vs_t", ";t  (GeV^{2});#Delta t  (GeV^{2})", 100, 0., 3.0, 100, 0., 0.);
	TProfile *p_de_t_vs_t = new TProfile("p_de_t_vs_t", ";t  (GeV^{2})", 300, 0., 3.0);

	for (unsigned int i = 0; i < 40000000; i++)
	{
		double t = gRandom->Rndm() * 3.5;
